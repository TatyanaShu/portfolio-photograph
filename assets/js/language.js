...
let langs = document.querySelectorAll('span');
...
elem.preventDefault()
      ...
      langs.forEach(i => {
        i.classList.remove('active-language')
        changeLanguage(i)
      }
      );
...
function changeLanguage(lang) {
    for (let key in i18Obj) {
        if (lang == key) {
            console.log('key', key)
            changeLanguageTags(key)
        }
    }
}
function changeLanguageTags(key) {
    langs.forEach(item => {
        for (let keyName in key) {
            if (item.dataset.name == keyName) {
